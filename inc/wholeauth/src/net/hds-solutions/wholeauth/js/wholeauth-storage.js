...
(function(root, factory) {
    ...
    _storage_size = 0, _storage_available = false, _ttl_timeout = null;
    ...
    function _load_storage() {
        var source = localStorage.wholeauthStorage;
        try {
            _storage = JSON.parse(source);
        } catch (E) {
            _storage = {};
        }
        _storage_size = localStorage.wholeauthStorage ? String(localStorage.wholeauthStorage).length : 0;
    }
    function _save() {
        try {
            localStorage.wholeauthStorage = JSON.stringify(_storage);
            _storage_size = localStorage.wholeauthStorage ? String(localStorage.wholeauthStorage).length : 0;
        } catch (E) {
            return E;
        }
    ...
    flush: function() {
        ...
        _storage = {};
        try {
            localStorage.removeItem("wholeauthStorage");
            return true;
        } catch (E) {
            return E;
        }
